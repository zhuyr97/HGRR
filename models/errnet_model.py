...
class ERRNetModel(ERRNetBase):
    ...
    def initialize(self, opt):
        ...
        self.net_i = arch.__dict__[self.opt.inet](in_channels, 3).to(self.device)  #inet : errnet
    ...
    def backward_G(self):
        ...
        self.loss_icnn_pixel_new = None #new add
        self.loss_icnn_pixel_new_H = None  # new add
        ...
        if self.aligned:
            ...
            self.loss_icnn_pixel_new = self.loss_dic['loss_new'].get_loss(
                self.output_i, self.target_t)*self.opt.lambda_newloss

            self.loss_icnn_pixel_new_H = self.loss_dic['loss_new_H'].get_loss(
                self.outputH_i, self.target_t) * self.opt.lambda_newloss_H

            self.loss_icnn_vgg = self.loss_dic['t_vgg'].get_loss(
                self.output_i, self.target_t)

            self.loss_G += self.loss_icnn_pixel+self.loss_icnn_vgg*self.opt.lambda_vgg \
                           + self.loss_icnn_pixel_new + self.loss_icnn_pixel_new_H
    ...
    def forward(self):
        ...
        output_i, outputH_i = self.net_i(input_i)

        self.output_i = output_i
        self.outputH_i = outputH_i
        return output_i,outputH_i
    ...
    def get_current_errors(self):
        ...
        if self.loss_icnn_pixel_new is not None:  # new add
            ret_errors['IPixel_new'] = self.loss_icnn_pixel_new.item()
        if self.loss_icnn_pixel_new_H is not None:  # new add
            ret_errors['IPixel_new_H'] = self.loss_icnn_pixel_new_H.item()

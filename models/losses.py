...
class HSVLoss_only_GTimg(nn.Module):
    def __init__(self):
        super(HSVLoss_only_GTimg, self).__init__()

    def forward(self, predict, target):
        target_hsv = colors.rgb_to_hsv(target.detach())#
        predict_hsv = predict     #torch.unsqueeze(predict_hsv[:,0,:,:], 1)
        target_hsv = torch.unsqueeze(target_hsv[:,0,:,:], 1)
        # print('target_hsv: ', target_hsv.shape)
...
def init_loss(opt, tensor):
    ...
    pixel_loss.initialize(MultipleLoss([nn.L1Loss(), GradientLoss()], [1,0.4])) #

    loss_dic['t_pixel'] = pixel_loss
    loss_dic['r_pixel'] = pixel_loss
    # new add
    pixelnew_loss =  ContentLoss()
    pixelnew_loss.initialize(MultipleLoss([HSVLoss()], [1]))
    loss_dic['loss_new'] = pixelnew_loss
    # new add
    pixelnew_loss_H = ContentLoss()
    pixelnew_loss_H.initialize(MultipleLoss([HSVLoss_only_GTimg()], [1]))
    loss_dic['loss_new_H'] = pixelnew_loss_H

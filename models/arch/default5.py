...
class GL_context_encoding(nn.Module):
    def __init__(self, channels, bias=True, mode=None):
        ...
        self.R3Attention = NonLocalBlock(channels) #R3Attention(in_dim=channels,residual = False)
...
class DRNet5_ori(torch.nn.Module):
    ...
    se_reduction=None, res_scale=1, bottom_kernel_size=3, pyramid=False,last_sigmoid=False):
        super(DRNet5_ori, self).__init__()
...
if __name__ == "__main__":
    ...
    fbar=DRNet5_ori(in_channels=4, out_channels=3, n_feats= 64, n_resblocks= 8, se_reduction=8, bottom_kernel_size=1, pyramid=True)

...
class DRNet(torch.nn.Module):
    ...
    se_reduction=None, res_scale=1, bottom_kernel_size=3, pyramid=False):
        ...
        self.res_module = nn.Sequential(*[GL_context_encoding1(n_feats, dilation=dilation_config[i],
                    norm=norm, act=act,se_reduction=se_reduction, res_scale=res_scale) for i in range(n_resblocks)])
        #
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x) #这一步downsample了
        x = self.res_module(x)

        x = self.deconv1(x)#这一步upsample了
        x = self.deconv2(x)
...
if __name__ == "__main__":
    x = torch.randn(2, 3, 127, 128)        #.cuda()#.to(torch.device('cuda'))
    fbar=DRNet(in_channels=3, out_channels=3, n_feats= 128, n_resblocks=18, norm=None, res_scale=0.1, se_reduction=8, bottom_kernel_size=1, pyramid=True)
    ...
    x = torch.randn(2, 64, 127, 128)
    fbar1 = ResidualBlock(channels=64,norm=None, res_scale=0.1, se_reduction=8)
    # print(fbar)
    y = fbar1(x)
    print(y.shape)
    print('-' * 50)
    print('#generator parameters:', sum(param.numel() for param in fbar1.parameters()))

    fbar2 = GL_context_encoding(64)
    # print(fbar)
    y = fbar2(x)
    print(y.shape)
    print('-' * 50)
    print('#generator parameters:', sum(param.numel() for param in fbar2.parameters()))

...
class GL_context_encoding1(nn.Module):
    def __init__(self, channels, dilation=1, norm=None, act=nn.ReLU(True), se_reduction=None, res_scale=1, bias=True,
                 mode=None):
...
class DRNet4(torch.nn.Module):
    def __init__(self, in_channels, out_channels, n_feats, n_resblocks, norm=nn.BatchNorm2d,
        ...
        self.res_module = nn.ModuleList([
            nn.Sequential(*[GL_context_encoding1(n_feats) for i in range(n_resblocks)]),
            nn.Sequential(*[GL_context_encoding1(n_feats) for i in range(n_resblocks)]),
            nn.Sequential(*[GL_context_encoding1(n_feats) for i in range(n_resblocks)])
        ])
...
if __name__ == "__main__":
    ...
    fbar = DRNet4(in_channels=4, out_channels=3, n_feats=72, n_resblocks=8, se_reduction=8, bottom_kernel_size=1,
                  pyramid=True)

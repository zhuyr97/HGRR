...
class SOS_boosting(nn.Module):
    def __init__(self, channels):
        super(SOS_boosting, self).__init__()
        self.conv = nn.Conv2d(channels, channels, kernel_size=3, padding=1,stride=1)
        self.lrelu = nn.LeakyReLU(0.2, inplace=True)
    def forward(self, fea1,fea2):
        fea_temp = fea1 + fea1
        fea_temp =self.lrelu(self.conv(fea_temp))
        enhanced_feature = fea_temp - fea1
...
class GL_context_encoding_T(nn.Module):
    def __init__(self, channels, bias=True, mode=None):
        super(GL_context_encoding_T, self).__init__()
        feature = channels
        self.Dalations_ResBlock = Dalations_ResBlock(in_channels=channels, out_channels=channels)
        self.R3Attention = R3Attention(in_dim=channels,residual = True)
        self.conv_tail = nn.Conv2d((feature*2), feature, kernel_size=1, padding=0, bias=bias)
...
class DRNet3(torch.nn.Module):
    def __init__(self, in_channels, out_channels, n_feats, n_resblocks, norm=nn.BatchNorm2d, 
    se_reduction=None, res_scale=1, bottom_kernel_size=3, pyramid=False,last_sigmoid=False):
        super(DRNet3, self).__init__()
        ...
        self.head_H = nn.Sequential(*[
            ConvLayer(conv, 4, n_feats, kernel_size=bottom_kernel_size, stride=1, norm=None,
                      act=act),
        ...
        self.SOS_fuse = nn.ModuleList([
            SOS_boosting(n_feats),
            SOS_boosting(n_feats),
            SOS_boosting(n_feats)
        ])
        ...
        self.deconv1_h = ConvLayer(deconv, n_feats, n_feats, kernel_size=4, stride=2, padding=1, norm=norm, act=act)
        if not pyramid:
            self.deconv2 = ConvLayer(conv, n_feats, n_feats, kernel_size=3, stride=1, norm=norm, act=act)
            self.deconv2_h = ConvLayer(conv, n_feats, n_feats, kernel_size=3, stride=1, norm=norm, act=act)
            self.deconv3 = ConvLayer(conv, n_feats, out_channels, kernel_size=1, stride=1, norm=None, act=act)
            self.deconv3_h = ConvLayer(conv, n_feats, 1, kernel_size=1, stride=1, norm=None, act=None)
        else:
            self.deconv2 = ConvLayer(conv, n_feats, n_feats, kernel_size=3, stride=1, norm=norm, act=act)
            self.deconv2_h = ConvLayer(conv, n_feats, n_feats, kernel_size=3, stride=1, norm=norm, act=act)
    ...
    def forward(self, x):
        init_x = x[:, :3, :, :]
        input_hsv = colors.rgb_to_hsv(init_x)  #
        input_hsv_h = torch.unsqueeze(input_hsv[:, 0, :, :], 1)

        input_x = x
        input_h_x = torch.cat([input_hsv_h, init_x], dim=1)
        ...
        x0 = self.SOS_fuse[0](x0,x0_h)

        x1 = self.res_module[1](x0)
        x1 = self.SOS_fuse[1](x1,x1_h)  # self.convs_1x1[1]( torch.cat([x1, x1_h], dim=1))

        x2 = self.res_module[2](x1)
        x2 = self.SOS_fuse[2](x2,x2_h)  #self.convs_1x1[2](torch.cat([x2, x2_h], dim=1))
        ...
        if self.last_sigmoid:
            x2_h = F.sigmoid(self.deconv3_h(x2_h))
        else:
            x2_h = self.deconv3_h(x2_h)
        return x2,x2_h

class DRNet3_T(torch.nn.Module):
    def __init__(self, in_channels, out_channels, n_feats, n_resblocks, norm=nn.BatchNorm2d,
    se_reduction=None, res_scale=1, bottom_kernel_size=3, pyramid=False,last_sigmoid=False):
        super(DRNet3_T, self).__init__()
        # Initial convolution layers
        conv = nn.Conv2d
        deconv = nn.ConvTranspose2d
        act = nn.ReLU(True)
        ...
        self.res_module = nn.ModuleList([
            nn.Sequential(*[GL_context_encoding_T(n_feats) for i in range(n_resblocks)]),
            nn.Sequential(*[GL_context_encoding_T(n_feats) for i in range(n_resblocks)]),
            nn.Sequential(*[GL_context_encoding_T(n_feats) for i in range(n_resblocks)])
            ])
...
if __name__ == "__main__":
    ...
    x = torch.randn(2, 4, 128, 128)        #.cuda()#.to(torch.device('cuda'))
    fbar=DRNet3(in_channels=4, out_channels=3, n_feats= 72, n_resblocks=8, se_reduction=8, bottom_kernel_size=1, pyramid=True)
